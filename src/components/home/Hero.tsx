    ...
    >
      ...
      <div className="relative flex h-full flex-col justify-end gap-10 px-5 pb-14 sm:px-8 sm:pb-20 lg:px-[88px] lg:pb-[110px]">
        <div className="max-w-[1180px]">
          <p className="animate-fade-in [animation-delay:.5s] mb-7 text-xs font-normal tracking-[0.32em] uppercase text-stone lg:text-3xl lg:tracking-[0.2em]">
        ...
        <div className="animate-fade-in [animation-delay:1s] flex flex-wrap items-end justify-between gap-10">
          <p className="font-lora max-w-[460px] text-base leading-[1.75] text-parchment-dim/90 text-pretty lg:max-w-[600px] lg:text-2xl lg:leading-[1.55]">
            I still shoot a stage the way I used to shoot a valley: wait for
            the light, let the movement stay in the frame. Concerts,
            festivals, sport, and the quiet minutes nobody else bothers to
            notice.
          </p>
          <div className="flex flex-wrap gap-3.5">
            <Link
              href="/book"
              className="inline-block rounded-full bg-parchment px-[38px] py-[18px] text-xs tracking-[0.2em] uppercase text-ink transition-all duration-700 hover:-translate-y-1 hover:bg-white lg:px-[42px] lg:py-[20px] lg:text-base"
            >
              Book an Event
            </Link>
            <a
              href="#work"
              className="inline-block rounded-full border border-parchment/30 px-[38px] py-[18px] text-xs tracking-[0.2em] uppercase text-parchment transition-all duration-700 hover:-translate-y-1 hover:bg-parchment/[0.08] hover:border-parchment/60 lg:px-[42px] lg:py-[20px] lg:text-base"
            >
              View Portfolio
            </a>
          </div>
